...
export function PlanScreen({ source, dest, onBack, onNext }: PlanScreenProps) {
  ...
  useEffect(() => {
    // Only start scan if we don't already have a result and not currently scanning
    if (!scanResult && !scanning) {
      console.log('PlanScreen: Starting new scan')
      startScan()
    } else if (scanResult) {
      console.log('PlanScreen: Using existing scan result')
    } else if (scanning) {
      console.log('PlanScreen: Scan already in progress')
    }
